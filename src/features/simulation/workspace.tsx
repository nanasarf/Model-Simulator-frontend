...
function ActiveSession({ id }: { id: string }) {
  ...
  return <SessionProvider value={context}><header className="page-heading"><p className="eyebrow">Live classroom</p><h1>Simulation session</h1><p className="support-id">{id}</p><ConnectionLabel status={status}/><BackgroundStatus active={state.isFetching}/></header>
    ...
    <dl className="session-facts"><div><dt>Model</dt><dd>{context.modelIdentifier}:{context.modelVersion}</dd></div><div><dt>Status</dt><dd>{context.status}</dd></div><div><dt>Round</dt><dd>{context.roundNumber}</dd></div><div><dt>Phase</dt><dd>{context.phase}</dd></div><div><dt>Team ID</dt><dd>{context.teamId || 'Not assigned'}</dd></div><div><dt>Assigned roles</dt><dd>{context.roleCodes.join(', ') || 'Not assigned'}</dd></div><div><dt>Capabilities</dt><dd>{context.capabilities?.join(', ') || 'Unavailable'}</dd></div><div><dt>Participant readiness</dt><dd>{context.participantReady === null ? 'Not a participant' : context.participantReady ? 'Ready' : 'Not ready'}</dd></div></dl>
